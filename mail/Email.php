...
class Email {
	...
	public function send($to, $from, $subject, $message, $html = false, $config = 'default') {
		...
		if($from === null) {
			$from = $this->mail->config->get("email.{$config}.sender");
		}
	...
	protected function build_mailer($config) {
		$type = $this->mail->config->get("email.{$config}.type",'native');
		switch ($type) {
			case 'smtp':
				
				// Create SMTP Transport
				$transport = \Swift_SmtpTransport::newInstance(
					$this->mail->config->get("email.{$config}.hostname"),
					$this->mail->config->get("email.{$config}.port",25)
				);
				
				// Set encryption if specified
				if ( ($encryption = $this->mail->config->get("email.{$config}.encryption",false)) !== false)
					$transport->setEncryption($encryption);
				
				// Set username if specified
				if ( ($username = $this->mail->config->get("email.{$config}.username",false)) !== false)
					$transport->setUsername($username);
					
				// Set password if specified
				if ( ($password = $this->mail->config->get("email.{$config}.password",false)) !== false)
					$transport->setPassword($password);
					
				// Set timeout, defaults to 5 seconds
				$transport->setTimeout($this->mail->config->get("email.{$config}.timeout", 5));
			...
			case 'sendmail':
				...
				$transport = \Swift_SendmailTransport::newInstance($this->mail->config->get("email.{$config}.sendmail_command", "/usr/sbin/sendmail -bs"));
			...
			case 'native':
				...
				$transport = \Swift_MailTransport::newInstance($this->mail->config->get("email.{$config}.mail_parameters","-f%s"));
